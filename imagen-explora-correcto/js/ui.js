...
export function getCurrentEditingHotspotId() {
    const input = document.getElementById('editing-hotspot-id');
    return input ? parseInt(input.value) : null;
}

export function getContentFromModal() {
    const type = document.getElementById('content-type').value;

    if (type === 'text') {
        return {
            type: 'text',
            title: document.getElementById('content-text-title').value,
            body: document.getElementById('content-text-body').value
        };
    } else if (type === 'link') {
        return {
            type: 'link',
            url: document.getElementById('content-link-url').value,
            text: document.getElementById('content-link-text').value
        };
    }
    return null;
}

// Asigna el ID del hotspot al input oculto cuando se abre el modal
export function showContentModal(hotspot) {
    console.log("Editando contenido para el hotspot:", hotspot.id);
    document.getElementById('editing-hotspot-id').value = hotspot.id;
    contentModal.classList.remove('hidden');
    contentModal.classList.add('flex');
}
